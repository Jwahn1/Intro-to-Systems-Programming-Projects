...
extern strlist_list_t * strlist_new() {
    ...
    // BUG forgot to
    memset(list, 0, sizeof(strlist_list_t));
    return list;
}
...
static strlist_node_t *new_node(char *string, strlist_node_t *prev, strlist_node_t *next) {
    // BUG
    // strlist_node_t *n = malloc(sizeof(strlist_list_t));
    // should be
    strlist_node_t *n = malloc(sizeof(strlist_node_t));
    // BUG (n == NULL) should be (n != NULL)
    if (n != NULL) {
        strncpy(n->string, string, sizeof(n->string));
        n->prev = prev;
        n->next = next;
    }
    return n;
...
extern void strlist_remove_front(strlist_list_t *list) {
    ...
    if (list->head != NULL) {
        ...
        list->head = list->head->next;
        // BUG (list->head != NULL) should be (list->head == NULL)
        if (list->head == NULL) {
            list->tail = NULL;
        } else {
            list->head->prev = NULL;
...
extern int ll_size(strlist_list_t *list) {
    ...
    // BUG strlist_node_t *ptr = list->tail should be strlist_node_t *ptr = list->head
...
extern void ll_remove(strlist_list_t *list, strlist_node_t *node) {
    ...
    // BUG strlist_remove_back(list) and strlist_remove_front(list)
    // should be switched
    if (node->prev == NULL) {
